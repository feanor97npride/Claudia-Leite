...
export default function RoadmapTimeline({ objetivos, atividades, currentWeekStart }: Props) {
  ...
  const rows = objetivos.flatMap((objetivo) =>
      ...
      {rows.length === 0 ? (
        ...
        <div className="overflow-x-auto">
          ...
          >
            ...
            {rows.map(({ objetivo, atividade, range }) => {
              const color = OBJETIVO_COLOR[objetivo.id];
              ...
              return (
                <div key={atividade.id} className="contents">
                  <div
                    ...
                    title={`${objetivo.entregaLabel} — ${atividade.name}`}
                  >
                    {atividade.name}
                  </div>
                  {before > 0 && <div className="border-b border-l border-slate-100" style={{ gridColumn: `span ${before}` }} />}
                  <div className="border-b border-l border-slate-100 relative py-0.5" style={{ gridColumn: `span ${range.span}` }}>
                    <div
                  ...
                  {after > 0 && <div className="border-b border-l border-slate-100" style={{ gridColumn: `span ${after}` }} />}
                </div>
              );
            })}
          </div>
